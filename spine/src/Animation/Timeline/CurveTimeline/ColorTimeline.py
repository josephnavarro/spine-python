...
class ColorTimeline(CurveTimeline):
    ...
    def setKeyframe(self, keyframeIndex: int, time: float, r: int, g: int, b: int, a: int) -> None:
        keyframeIndex *= self.FRAME_SPACING
        self.frames[keyframeIndex + 0] = time
    ...
    def apply(self, skeleton, time, alpha):
        if time < self.frames[0]:
            ...
            if time >= self.frames[self.LAST_FRAME_TIME]:
                ...
                frameTime = self.frames[frameIndex - 0]

                percent: float = 1 - (time - frameTime) / (self.frames[frameIndex + self.LAST_FRAME_TIME] - frameTime)
                if percent < 0:
                    percent = 0
                if percent > 255:
                    percent = 255
